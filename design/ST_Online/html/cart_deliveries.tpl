{if $deliveries}
		...
		<div class="delivery mb-3">
			<div class="h3 text-caps mb-sm" data-language="cart_delivery">{$lang->cart_delivery}</div>
			{foreach $deliveries as $delivery}
				<div class="delivery_item">
					...
					<div class="delivery_description d_{$delivery->id}">
	...
	{foreach $deliveries as $delivery}
		{if $delivery->payment_methods}
			<div class="fn_delivery_payment" id="fn_delivery_payment_{$delivery->id}"{if $delivery@iteration != 1} style="display:none"{/if}>
				<div class="delivery">
					<div class="h3 text-caps mb-sm" data-language="cart_payment">{$lang->cart_payment}</div>
					{foreach $delivery->payment_methods as $payment_method}
						<div class="delivery_item">
							<label class="delivery_label{if $payment_method@first} active{/if}" for="payment_{$delivery->id}_{$payment_method->id}">
								<input class="input_delivery" onclick="payment_click({$payment_method->id})" id="payment_{$delivery->id}_{$payment_method->id}" type="radio" name="payment_method_id" value="{$payment_method->id}"{if $delivery@first && $payment_method@first} checked{/if} />
								<span class="delivery_name">
